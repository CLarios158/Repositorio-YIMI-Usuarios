...
import { ActivityIndicator, Alert, StyleSheet, View, Text, TouchableOpacity, TouchableNativeFeedback, TextInput, StatusBar, Image } from 'react-native';
...
class VerificationCode extends Component {
  ...
  validateCode = async () => {
    ...
    try {
      const res = await axios.post(`${Globals.server}:${Globals.port}/validar_codigo`, { telefono: this.state.telefono, codigo: this.state.code.join('') });
      console.log(res)
      if (res.status == 200) {
        if (res.data.respuesta == 0) {
          ...
          //this.refs.codeInputRef1.clear();
          this.props.navigation.navigate('AddPersonalInformation', { telefono: this.state.telefono });
        }
      } else {
            ...
            <Grid>
              ...
              <Row style={{ marginTop: 40 }}>
                <Col style={{ display: 'flex', alignContent: 'center', alignItems: 'center', marginLeft: 85, marginRight: 85 }}>
                  <Button onPress={this.validateCode} block style={{ backgroundColor: '#ff8834' }}><Text style={{ fontFamily: 'aller-bd', textTransform:'capitalize' }}>Siguiente</Text></Button>
                </Col>
              </Row>
            </Grid>
          </Content>
